...
static void *can_new(mybmm_config_t *conf,...) {
	can_info_t *info;
	char interface[CAN_INTERFACE_LEN], *opts;
	int bitrate;
#ifndef JBDTOOL
        struct cfg_proctab cantab[] = {
		{ interface, "bitrate", "CAN bitrate", DATA_TYPE_INT,&bitrate,0,"500000" },
		CFG_PROCTAB_END
	};
#endif
	va_list ap;
	...
	opts = va_arg(ap,char *);
	va_end(ap);
	dprintf(3,"target: %s, opts: %s\n", target, opts);

	interface[0] = 0;
	strncat(interface,target,CAN_INTERFACE_LEN-1);
#ifndef JBDTOOL
	dprintf(1,"cfg: %p\n", conf->cfg);
	cfg_get_tab(conf->cfg,cantab);
        if (debug >= 3) cfg_disp_tab(cantab,0,1);
#else
	bitrate = atoi(opts);
#endif
	dprintf(3,"bitrate: %d\n",bitrate);
...
#if 0
static int can_get(int s, int id, void *dptr, int len) {
	struct can_frame frame;
	int nbytes;

	dprintf(3,"s: %d, id: %03x, dptr: %p, len: %d\n", s, id, dptr, len);
	memset(&frame,0,sizeof(struct can_frame));
	frame.can_id = id;
//	memcpy(&frame.data,data,len);
	nbytes = write(s, &frame, sizeof(struct can_frame));
	dprintf(3,"write nbytes: %d\n", nbytes);
	if (nbytes < 0) {
		perror("get_can write");
		return -1;
	}

	nbytes = read(s, &frame, sizeof(struct can_frame));
	dprintf(3,"read nbytes: %d\n", nbytes);
	if (nbytes < 0) {
		perror("get_can read");
		return -1;
	}
	...
	if (debug >= 5) bindump("FROM BMS",&frame,sizeof(struct can_frame));
	memcpy(dptr,&frame.data,len);
	return frame.can_dlc;
}

static int can_recv(int s, int id, void *dptr, int len) {
	struct can_frame frame;
//	struct can_filter f = { id, CAN_SFF_MASK };
	int nbytes;
...
#if 0
	/* We only want our ID */
	if (setsockopt(s, SOL_CAN_RAW, CAN_RAW_FILTER, &f, sizeof(f)) < 0) {
		perror("si_can_get_info: setsockopt");
		return 1;
	}
#endif
...
//	memset(&frame,0,sizeof(struct can_frame));
	...
	do {
		nbytes = read(s, &frame, sizeof(struct can_frame));
		dprintf(3,"frame: id: %x\n", frame.can_id);
		dprintf(3,"read nbytes: %d\n", nbytes);
		if (nbytes < 0) {
//			perror("get_recv read");
			return 1;
		}
		if (nbytes < sizeof(struct can_frame)) {
			fprintf(stderr, "read: incomplete CAN frame\n");
		...
		dprintf(5,"nbytes: %d, id: %x, frame.can_id: %x\n", nbytes, id, frame.can_id);
	} while(id != 0xFFFF && frame.can_id != id);
	if (debug >= 5) bindump("FROM BMS",&frame,sizeof(struct can_frame));
	memcpy(dptr,&frame,sizeof(frame));
	/* Clear the filter! */
